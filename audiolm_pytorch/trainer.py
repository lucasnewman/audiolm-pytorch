...
class SoundStreamTrainer(nn.Module):
    ...
    def train_step(self):
        ...
        for _ in range(self.grad_accum_every):
            ...
            loss, (recon_loss, multi_spectral_recon_loss, adversarial_loss, feature_loss, all_commitment_loss) = self.soundstream(wave, use_discriminators = self.use_discriminators, return_loss_breakdown = True)
        ...
        if self.use_discriminators:
            ...
            accum_log(logs, dict(stft = 0))

        # build pretty printed losses

        losses_str = f"{steps}: loss: {logs['loss']:.3f}, recon loss: {logs['recon_loss']:.3f}"
        if log_losses:
        ...
        losses_str += f" | multi_spectral_recon loss: {logs['multi_spectral_recon_loss']:.3f}"
        losses_str += f" | adversarial loss: {logs['adversarial_loss']:.3f}"
        losses_str += f" | feature loss: {logs['feature_loss']:.3f}"
        losses_str += f" | commitment loss: {logs['all_commitment_loss']:.3f}"
        losses_str += f" | stft_discr loss: {logs['stft']:.3f}"
        
        # compute the l2 norm of each codebook

        codebooks = self.unwrapped_soundstream.rq.codebooks.cpu().detach()
        for ind, codebook in enumerate(codebooks.squeeze(0).unbind(dim = 0)):
            losses_str += f" | codebook {ind + 1} l2 norm: {vector_norm(codebook):.3f}"
        ...
        if self.is_main and not (steps % self.save_results_every):
            ...
            for model, label in models:
                ...
                # show mel spectrogram plots for the reference and reconstruction
                ...
                for ind, one_wave in enumerate(wave.unbind(dim = 0)):
                    if ind != 0:
                        continue
                        
                    mel_spectrogram = mel_transform(one_wave.cpu().detach()).squeeze(dim = 0)
                    axs[0].imshow(mel_spectrogram.log2(), aspect='auto', origin='lower')
                    axs[0].set_title(f'Reference')
                    fig.colorbar(axs[0].imshow(mel_spectrogram.log2(), aspect='auto', origin='lower'), ax=axs[0], format='%+2.0f dB')
                    
                for ind, recon in enumerate(recons.unbind(dim = 0)):
                    if ind != 0:
                        continue
                        
                    mel_spectrogram = mel_transform(recon.cpu().detach()).squeeze(dim = 0)
                    axs[1].imshow(mel_spectrogram.log2(), aspect='auto', origin='lower')
                    axs[1].set_title(f'Reconstruction')
                    fig.colorbar(axs[1].imshow(mel_spectrogram.log2(), aspect='auto', origin='lower'), ax=axs[1], format='%+2.0f dB')
